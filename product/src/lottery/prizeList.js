...
const MAX_TOP = 300, MAX_WIDTH = document.body.clientWidth;
...
let prizeElement = {},
  	lasetPrizeIndex = PRIZES.length - 1;
class DanMu {
	constructor(option) {
...
function showPrizeList(currentPrizeIndex) {
	let currentPrize = getCurrentPrize(currentPrizeIndex);
...
let setPrizeData = (function() {
	return function(currentPrizeIndex, count, isInit) {
		...
		if (!elements && currentPrizeIndex !== -1) {
			elements = {
		...
		if (lasetPrizeIndex !== currentPrizeIndex) {
			...
			elements && elements.box && elements.box.classList.add("shine");
		...
		elements && elements.bar && (elements.bar.style.width = percent * 100 + "%");
		elements && elements.text && (elements.text.textContent = count + "/" + totalCount);
